...
class ProgramDatabase:
    ...
    def seed_population(self, initial_code: str, fitness: float = -float("inf")) -> None:
        """
    ...
    def sample_for_context(self, num_samples: int) -> List[Program]:
        ...
        for program in self.archive:
            fitness_weight = (program.fitness - min(p.fitness for p in self.archive)) / \
                           (max(p.fitness for p in self.archive) - min(p.fitness for p in self.archive) + 1e-6)
    ...
    def _prune_map_elites(self) -> None:
        ...
        for length_bucket in sorted(groups.keys()):
            bucket = sorted(groups[length_bucket], key=lambda p: p.fitness, reverse=True)
